	...
	int block_size)
{
	...
	cache->pages = block_size >> PG_SHIFT;
	...
	for (i = 0; i < entries; i++) {
		...
		for (j = 0; j < cache->pages; j++) {
			entry->data[j] = kmalloc(PG_SIZE, GFP_KERNEL);
		...
		int offset, int length)
{
	...
	while (offset < entry->length) {
		void *buff = entry->data[offset / PG_SIZE]
				+ (offset % PG_SIZE);
		int bytes = min_t(int, entry->length - offset,
				PG_SIZE - (offset % PG_SIZE));
...
{
	int pages = (length + PG_SIZE - 1) >> PG_SHIFT;
	int i, res;
	...
	for (i = 0; i < pages; i++, buffer += PG_SIZE)
		data[i] = buffer;
