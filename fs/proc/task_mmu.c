...
void task_mem(struct seq_file *m, struct mm_struct *mm)
{
	...
	text = PTE_ALIGN(mm->end_code) - (mm->start_code & ~PTE_MASK);
	text = min(text, mm->exec_vm << PTE_SHIFT);
	lib = (mm->exec_vm << PTE_SHIFT) - text;
			 ...
			 unsigned long *data, unsigned long *resident)
{
	...
	*text = (PTE_ALIGN(mm->end_code) - (mm->start_code & ~PTE_MASK)) >> PTE_SHIFT;
...
static void show_vma_header_prefix(struct seq_file *m,
				   ...
				   vm_flags_t flags, unsigned long long pteoff,
...
show_map_vma(struct seq_file *m, struct vm_area_struct *vma)
{
	...
	if (vma->vm_file) {
		...
		ino = inode->i_ino;
		pteoff = ((loff_t)vma->vm_pteoff) << PTE_SHIFT;
	}

	start = vma->vm_start;
	end = vma->vm_end;
	show_vma_header_prefix(m, start, end, flags, pteoff, dev, ino);
...
static int do_procmap_query(struct mm_struct *mm, void __user *uarg)
{
	...
	if (vma->vm_file) {
		...
		karg.vma_offset = ((__u64)vma->vm_pteoff) << PTE_SHIFT;
		...
		bool present)
{
	...
	unsigned long size = compound ? nr * PG_SIZE : PTE_SIZE;
	bool exclusive;
	int mapcount;
	 ...
	 */
	for (i = 0; i < nr; i++, page++) {
		unsigned long pss = PTE_SIZE << PSS_SHIFT;
		...
		if (mapcount >= 2)
			pss /= mapcount;
		smaps_page_accumulate(mss, folio, PTE_SIZE, pss,
			  ...
			  __always_unused int depth, struct mm_walk *walk)
{
	...
	mss->swap += shmem_partial_swap_usage(walk->vma->vm_file->f_mapping,
					      linear_pte_index(vma, addr) / PTES_PER_PAGE,
					      linear_pte_index(vma, end) / PTES_PER_PAGE);
...
static void smaps_pte_hole_lookup(unsigned long addr, struct mm_walk *walk)
{
	...
	if (walk->ops->pte_hole) {
		/* depth is not used */
		smaps_pte_hole(addr, addr + PTE_SIZE, 0, walk);
	}
#endif
}
		...
		struct mm_walk *walk)
{
	...
	if (pte_present(ptent)) {
		...
		if (softleaf_is_swap(entry)) {
			...
			if (mapcount >= 2) {
				u64 pss_delta = (u64)PTE_SIZE << PSS_SHIFT;

				do_div(pss_delta, mapcount);
				mss->swap_pss += pss_delta;
			} else {
				mss->swap_pss += (u64)PTE_SIZE << PSS_SHIFT;
			}
			   ...
			   struct mm_walk *walk)
{
	...
	for (; addr != end; pte++, addr += PTE_SIZE)
			    ...
			    __always_unused int depth, struct mm_walk *walk)
{
	...
	while (addr < end) {
		...
		for (; addr < hole_end; addr += PTE_SIZE) {
		...
		for (; addr < min(end, vma->vm_end); addr += PTE_SIZE) {
			err = add_to_pagemap(&pme, pm);
			if (err)
				goto out;
		}
...
{
	unsigned int idx = (addr & ~PMD_MASK) >> PTE_SHIFT;
	u64 flags = 0, frame = 0;
	pmd_t pmd = *pmdp;
	struct page *page = NULL;
	struct folio *folio = NULL;
...
populate_pagemap:
	for (; addr != end; addr += PTE_SIZE, idx++) {
		u64 cur_flags = flags;
		pagemap_entry_t pme;
			    ...
			    size_t count, loff_t *ppos)
{
	...
	pm.len = (PAGEMAP_WALK_SIZE >> PTE_SHIFT);
	...
	if (svpfn <= (ULONG_MAX >> PTE_SHIFT)) {
		unsigned long end;

		ret = mmap_read_lock_killable(mm);
		if (ret)
			goto out_free;
		start_vaddr = untagged_addr_remote(mm, svpfn << PTE_SHIFT);
		mmap_read_unlock(mm);

		end = start_vaddr + ((count / PM_ENTRY_BYTES) << PTE_SHIFT);
				       ...
				       unsigned long addr, unsigned long end)
{
	...
	p->found_pages -= (end - addr) / PG_SIZE;
}
#endif
			       ...
			       unsigned long addr, unsigned long *end)
{
	...
	n_pages = (*end - addr) / PG_SIZE;
	if (check_add_overflow(p->found_pages, n_pages, &total_pages) ||
	    total_pages > p->arg.max_pages) {
		size_t n_too_much = total_pages - p->arg.max_pages;
		*end -= n_too_much * PG_SIZE;
		n_pages -= n_too_much;
		ret = -ENOSPC;
	}
				  ...
				  unsigned long end, struct mm_walk *walk)
{
	...
	if ((p->arg.flags & PM_SCAN_WP_MATCHING) && !p->vec_out) {
		...
		for (addr = start; addr != end; pte++, addr += PTE_SIZE) {
			pte_t ptent = ptep_get(pte);
	...
	if (!p->arg.category_anyof_mask && !p->arg.category_inverted &&
		...
		for (addr = start; addr < end; pte++, addr += PTE_SIZE) {
			unsigned long next = addr + PTE_SIZE;
			pte_t ptent = ptep_get(pte);
				 ...
				 unsigned long uarg)
{
	...
	if (!IS_ALIGNED(arg->start, PTE_SIZE))
		return -EINVAL;
	...
	arg->end = ALIGN(arg->end, PTE_SIZE);
	arg->walk_end = 0;
	if (!arg->max_pages)
...
static int pagemap_scan_init_bounce_buffer(struct pagemap_scan_private *p)
{
	...
	p->vec_buf_len = min_t(size_t, PAGEMAP_WALK_SIZE >> PTE_SHIFT,
			       p->arg.vec_len);
