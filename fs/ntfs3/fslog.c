			 ...
			 struct RECORD_PAGE_HDR **buffer, bool *usa_error)
{
	...
	if (page_buf->rhdr.sign != NTFS_FFFF_SIGNATURE)
		ntfs_fix_post_read(&page_buf->rhdr, PG_SIZE, false);
...
int log_replay(struct ntfs_inode *ni, bool *initialized)
{
	...
	log->page_size = norm_file_page(PG_SIZE, &log->l_size, true);
#else
	log->page_size = norm_file_page(PG_SIZE, &log->l_size, false);
#endif
	if (!log->page_size) {
		err = -EINVAL;
		goto out;
