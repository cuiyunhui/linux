...
static inline void pmd_populate_kernel(struct mm_struct *mm,
				       pmd_t *pmd, pte_t *pte)
{
	paravirt_alloc_pte(mm, __pa(pte) >> PTE_SHIFT);
				...
				struct page *pte)
{
	...
	set_pmd(pmd, __pmd(((pteval_t)pfn << PTE_SHIFT) | _PAGE_TABLE));
}
...
{
	paravirt_alloc_pud(mm, __pa(pud) >> PTE_SHIFT);
...
static inline void pgd_populate(struct mm_struct *mm, pgd_t *pgd, p4d_t *p4d)
{
	if (!pgtable_l5_enabled())
		return;
	paravirt_alloc_p4d(mm, __pa(p4d) >> PTE_SHIFT);
