		...
		struct gup_test *gup)
{
	...
	for (addr = gup->addr; addr < gup->addr + gup->size; addr = next) {
		...
		next = addr + nr * PG_SIZE;
		if (next > gup->addr + gup->size) {
			next = gup->addr + gup->size;
			nr = (next - addr) / PG_SIZE;
		}
...
static inline int pin_longterm_test_start(unsigned long arg)
{
	...
	if (args.flags &
		...
		return -EINVAL;
	if (!IS_ALIGNED(args.addr | args.size, PG_SIZE))
		return -EINVAL;
	if (args.size > LONG_MAX)
		return -EINVAL;
	nr_pages = args.size / PG_SIZE;
	if (!nr_pages)
		return -EINVAL;
	...
	while (nr_pages - pin_longterm_test_nr_pages) {
		...
		addr = args.addr + pin_longterm_test_nr_pages * PG_SIZE;
