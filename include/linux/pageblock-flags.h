...
/*
 * pageblock_nr_pages is in allocator pages (PG_SIZE). PFNs are in PTE_SIZE
 * units when PG_SIZE and PTE_SIZE are decoupled, so convert pageblock
 * alignment helpers to PFN units explicitly.
 */
#define pageblock_nr_ptes	PAGES_TO_PTES(pageblock_nr_pages)
#define pageblock_align(pfn)	ALIGN((pfn), pageblock_nr_ptes)
#define pageblock_aligned(pfn)	IS_ALIGNED((pfn), pageblock_nr_ptes)
#define pageblock_start_pfn(pfn)	ALIGN_DOWN((pfn), pageblock_nr_ptes)
#define pageblock_end_pfn(pfn)		ALIGN((pfn) + 1, pageblock_nr_ptes)
