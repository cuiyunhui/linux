 ...
 * very large NR_CPUS.
 *
 *  Use PG_SIZE as a minimum for smaller configurations while avoiding
 *  unsigned comparison to -1.
 */
#define CPUMAP_FILE_MAX_BYTES  (((NR_CPUS * 9)/32 > PG_SIZE) \
					? (NR_CPUS * 9)/32 - 1 : PG_SIZE)
#define CPULIST_FILE_MAX_BYTES  (((NR_CPUS * 7)/2 > PG_SIZE) ? (NR_CPUS * 7)/2 : PG_SIZE)
